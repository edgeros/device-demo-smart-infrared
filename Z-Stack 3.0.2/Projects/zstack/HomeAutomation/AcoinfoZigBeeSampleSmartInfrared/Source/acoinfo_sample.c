...
#define ACOINFO_SAMPLE_HUMAN_ATTR_ID        ACOINFO_ZB_AIO_ATTR_BASE + ACOINFO_SAMPLE_HUMAN_ATTR_CHAN_NUM

#define ACOINFO_SAMPLE_LDR_ATTR_ID          ACOINFO_ZB_AIO_ATTR_BASE + ACOINFO_SAMPLE_LDR_ATTR_CHAN_NUM

#define ACOINFO_SAMPLE_REP_ATTR_ID          ACOINFO_ZB_MEM_ATTR_BASE + ACOINFO_SAMPLE_REP_ATTR_CHAN_NUM

#define SAMPLEAPP_DIO_ATTR_DATA_LEN         1

#define SAMPLEAPP_AIO_ATTR_DATA_LEN         4

#define SAMPLEAPP_MEM_ATTR_DATA_LEN         4
...
*
*/
#define DATA_PIN_6 P0_6            //定义P0.6 红外传感器
#define DATA_PIN_5 P0_5            //定义P0.5 光敏传感器
...
static bool  LdrStatus = false;  // 光敏状态 
static bool  HumanStatus = false; // 人体状态

static float vddValue = 0;
static uint32 repValue = 0;
/*********************************************************************
* LOCAL FUNCTIONS
*/
...
void acoinfoSample_Init( byte task_id )
{
  ...
  * 初始化 人体红外传感器，光敏传感器
  */
  P0SEL &= 0x9F; //设置P0.5,P0.6端口为普通IO
  P0DIR &= 0x9F; //设置P0.5,P0.6端口为输入
...
static void acoinfoSample_HandleKeys( byte shift, byte keys )
{
  ...
  //s2有被按下
  if ( keys == HAL_KEY_STATE_NORMAL )
  {
    debugMessage( "chick button:S2 \r\n" );
  }
}
...
static void acoinfoSample_ReportEvent( void )
{
  ...
  zclReportCmd_t *iotReportCmd;
  zclReportCmd_t *repReportCmd;

  
  pReportCmd = (zclReportCmd_t *)osal_mem_alloc( sizeof(zclReportCmd_t) + sizeof(zclReport_t) );
  iotReportCmd = (zclReportCmd_t *)osal_mem_alloc( sizeof(zclReportCmd_t) + sizeof(zclReport_t)*2);
  repReportCmd = (zclReportCmd_t *)osal_mem_alloc( sizeof(zclReportCmd_t) + sizeof(zclReport_t) );
  if (pReportCmd == NULL || iotReportCmd == NULL || repReportCmd == NULL)
  {
    return;
  }
  ...
  */
  ...
  iotReportCmd->numAttr              = 2;
  iotReportCmd->attrList[0].attrID   = ACOINFO_SAMPLE_HUMAN_ATTR_ID;
  iotReportCmd->attrList[0].dataType = ZCL_DATATYPE_UINT8;
  iotReportCmd->attrList[0].attrData = (uint8 *)&HumanStatus;
  
  iotReportCmd->attrList[1].attrID   = ACOINFO_SAMPLE_LDR_ATTR_ID;
  iotReportCmd->attrList[1].dataType = ZCL_DATATYPE_UINT8;
  iotReportCmd->attrList[1].attrData = (uint8 *)&LdrStatus;
  if ( zcl_SendReportCmd( ACOINFO_ZB_ENDPOINT, &dstAddr,
                         ACOINFO_ZB_DIO_CLUSTER, iotReportCmd,
                         ZCL_FRAME_SERVER_CLIENT_DIR, TRUE, seqNum++ ) == ZCL_STATUS_SUCCESS )
  {
    debugMessage( ">report Human:%d Ldr:%d  \r\n",LdrStatus,HumanStatus);
  }
  
  //发送Rep数量
  repValue++;
  repReportCmd->numAttr              = 1;
  repReportCmd->attrList[0].attrID   = ACOINFO_SAMPLE_REP_ATTR_ID;
  repReportCmd->attrList[0].dataType = ZCL_DATATYPE_UINT32;
  repReportCmd->attrList[0].attrData = (uint8 *)&repValue;
  
  if ( zcl_SendReportCmd( ACOINFO_ZB_ENDPOINT, &dstAddr,
                         ACOINFO_ZB_MEM_CLUSTER, repReportCmd,
                         ZCL_FRAME_SERVER_CLIENT_DIR, TRUE, seqNum++ ) == ZCL_STATUS_SUCCESS )
  {
    debugMessage( ">report report number:%d \r\n",repValue);
  }
  
  /** **/
  osal_mem_free( pReportCmd );
  osal_mem_free( iotReportCmd );
  osal_mem_free( repReportCmd );
}
...
static ZStatus_t acoinfoSample_ReadWriteCallBack( uint16 clusterId, uint16 attrId, uint8 oper, uint8 *pValue, uint16 *pLen )
{
  debugMessage( "gateway send message clusterId:%f  attrId:%f!!!!\r\n",clusterId,attrId );
...
static ZStatus_t acoinfoSample_HandleDioData( uint16 attrId, uint8 oper, uint8 *pValue, uint16 *pLen )
{
  ...
  case ZCL_OPER_READ:
    ...
    * 读取光敏状态
    */
    if ( attrId == ACOINFO_SAMPLE_LDR_ATTR_ID )
    {
      *pValue = LdrStatus;
      *pLen   = SAMPLEAPP_DIO_ATTR_DATA_LEN;
      ret    = ZCL_STATUS_SUCCESS;
    ...
    * 读取人体状态
    */      
    if ( attrId == ACOINFO_SAMPLE_HUMAN_ATTR_ID )
    {
      *pValue = HumanStatus;
      *pLen   = SAMPLEAPP_DIO_ATTR_DATA_LEN;
      ret    = ZCL_STATUS_SUCCESS;
...
static ZStatus_t acoinfoSample_HandleMemData( uint16 attrId, uint8 oper, uint8 *pValue, uint16 *pLen )
{
  ...
  switch( oper )
  {
    ...
    */
  case ZCL_OPER_READ:
    if ( attrId == ACOINFO_SAMPLE_REP_ATTR_ID )
    {
      vddValue = acoinfoSample_VddValueGet();
      
      *(float *)pValue = repValue;
      *pLen = SAMPLEAPP_MEM_ATTR_DATA_LEN;
      ret  = ZCL_STATUS_SUCCESS;
    }
    break;
