...
static const uint8 acoinfoSample_ManufacturerName[] = { 17, 'e','d','g','e','r','o','s','.','s','m','a','r','t','.','h','u','m'}; // manufacturer 名

/*
 * Acoinfo generic cluster common attribute
 */
static uint32 dioActiveMask   = 0x7; // DIO 有效通道掩码
...
static uint32  memActiveMask   = 0x1; // MEM 有效通道掩码
static uint32  memWritableMask = 0x1; // MEM 可写通道掩码
/*********************************************************************
 * GLOBAL VARIABLES
 */
/*********************************************************************
...
CONST zclAttrRec_t acoinfoSample_Attrs[] =
{
  ...
  ACOINFO_ZB_DIO( ACOINFO_SAMPLE_LED_ATTR_CHAN_NUM,  ACCESS_CONTROL_READ | ACCESS_CONTROL_WRITE )
   ...
   /*
   * 添加一个开关量，即 人体红外感应器(只读)
   */
  ACOINFO_ZB_DIO( ACOINFO_SAMPLE_HUMAN_ATTR_CHAN_NUM,  ACCESS_CONTROL_READ )
    
   /*
   * 添加一个开关量，即 红外感应器(只读)
   */
  ACOINFO_ZB_DIO( ACOINFO_SAMPLE_LDR_ATTR_CHAN_NUM,  ACCESS_CONTROL_READ )
  ...
  /*
   * 添加一个MEM内存，即发包数量所使用的属性（只读）
   */
  ACOINFO_ZB_MEM( ACOINFO_SAMPLE_REP_ATTR_CHAN_NUM,  ACCESS_CONTROL_READ | ACCESS_CONTROL_WRITE )
